...
def make_feats():
    if os.path.exists(ROOT / 'data/processed/user_feature.pkl'):
        all_df = pd.read_pickle(ROOT / 'data/processed/user_feature.pkl')
    else:
        df = pd.read_csv(ROOT / 'data/raw/train.csv')
        tdf = pd.read_csv(ROOT / 'data/raw/testA.csv')
        all_df = pd.concat([df, tdf]).reset_index(drop=True)
        
        all_df = split_date_part(all_df)
        all_df = revise_cum_col(all_df)
        ...
        all_df = make_last_day_half_roi(all_df)
        # diff feats
        ...
        for col in TG_ENC_COLS:
            ...
            all_df = target_enc(all_df, col, target='LAST_DAY_HALF_ROI')
        
        all_df.to_pickle(ROOT / 'data/processed/user_feature.pkl')
...
if __name__ == '__main__':
    ...
    est_list = get_k_fold_ests(all_df, fea_cols)
    for est in est_list:
        save_path = ROOT / 'data/submit/model-{}_seed-{}.csv'.format(est.__class__.__name__, est.params['random_state'])
